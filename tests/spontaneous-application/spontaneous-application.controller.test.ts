...
vi.mock('@/services/spontaneous-application.services', () => ({
  findAll: mockFindAll,
...
describe('spontaneousApplicationController', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('getAll', () => {
    it('returns all applications', async () => {
      ...
      await spontaneousApplicationController.getAll(req, res, next);
  ...
  describe('getById', () => {
    it('returns the application when found', async () => {
      ...
      await spontaneousApplicationController.getById(req, res, next);
    ...
    it('calls next with NotFoundError when not found', async () => {
      ...
      spontaneousApplicationController.getById(req, res, next);
      await flush();
  ...
  describe('create', () => {
    it('creates and returns 201', async () => {
      const req = mockReq({
        body: { first_name: 'Jane', last_name: 'Smith', email: 'jane@example.com', message: 'Hi' },
      });
      const res = mockRes();
      const next = vi.fn();
      ...
      await spontaneousApplicationController.create(req, res, next);
  ...
  describe('update', () => {
    it('updates and returns the application', async () => {
      ...
      mockUpdate.mockResolvedValue({ ...fakeApplication, message: 'Updated' });

      await spontaneousApplicationController.update(req, res, next);
  ...
  describe('remove', () => {
    it('deletes and returns 204', async () => {
      ...
      await spontaneousApplicationController.remove(req, res, next);
